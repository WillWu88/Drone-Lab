...
Aileron_A = expm(A_dt_v .* (1/200));
fun2 = exp(Aileron_A.*t);
Aileron_B = double(int(fun2,0,1/200))*B_dt_v;
Aileron_A = Aileron_A(1:2,1:2);
Aileron_B = Aileron_B(1:2);
% U theta q (Elevator) needs to be fixed
Elevator_A = expm(A_dt_u .* (1/200));
fun3 = exp(Elevator_A .*t);
Elevator_B = double(int(fun3,0,1/200))*B_dt_u;
